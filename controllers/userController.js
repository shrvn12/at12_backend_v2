...
const getUserInfo = async (req, res) => {
    try {
        const userData = await userService.getUserInfo(req.user._id);
        res.json(userData);
    } catch (error) {
...
const getLikedSongs = async (req, res) => {
    try {
        const likedSongs = await userService.getLikedSongs(req.user._id);
        res.json(likedSongs);
    } catch (error) {
...
const likeSong = async (req, res) => {
    try {
        const result = await userService.likeSong(req.user._id, req.body.songId);
        res.json(result);
    } catch (error) {
...
const unlikeSong = async (req, res) => {
    try {
        const result = await userService.unlikeSong(req.user._id, req.body.songId);
        res.json(result);
    } catch (error) {
